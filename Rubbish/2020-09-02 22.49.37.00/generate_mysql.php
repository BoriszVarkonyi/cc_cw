...
for ($i=0; $i < count($allwc); $i++) { 
    ...
    $maxinperiod = $dayscontrolon[1] * 2;


    $query = "CREATE TABLE `ccdatabase`.`WC_$dayscontrolon[0]_$comp_id` ( `hour` INT(11) NOT NULL , /*in_hour INT(11) NOT NULL ,*/ `period` INT(11) NOT NULL , `ids` VARCHAR($maxinperiod) NOT NULL ) ENGINE = InnoDB;";
    $query_do = mysqli_query($connection, $query);
...
while ($kezd < $fejez){
    ...
    $period_to_mysql =substr($period, 0 , strlen($period) - 1);
    $query = "INSERT INTO `WC_$dayscontrolon[0]_$comp_id`(`hour`, `period`, `ids`) VALUES $period_to_mysql,(999,1,$values_hour_minus);";
    $query_do = mysqli_query($connection, $query);
    //print_r($dayscontrolon);
    //print_r($one);
    echo $period_to_mysql;
